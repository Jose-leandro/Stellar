...
async function buscarInformacoesBancoDados() {
  ...
  const pool = createPool({
    ...
    connectionString: 'postgres://default:DEsoX6JqN1AM@ep-green-silence-a4qtd6to-pooler.us-east-1.aws.neon.tech:5432/verceldb?sslmode=require&pgbouncer=true&connect_timeout=15'
  })
...
export async function criptacaoDados(key, value, chaveCriptografica) {
  ...
  const qualDadoCriptografado = CryptoJS.AES.encrypt(value, chaveCriptografica).toString()
  ...
  if (!(key in dadosCriptogrados)) {
    ...
    console.log(`Dado "${key}" adicionado com valor "${qualDadoCriptografado}".`)
  } else {
...
export async function criptografiaSenha(dadosUsurios) {
  ...
  for (const key in dadosUsurios) {
    ...
    try {
     ...
     const chaveCriptografica: string | undefined = 'k1h8w3q7i5l2c6h8e1z3r7p0t3m0s1t9x2i0a1c5j8e3b5x1m0y2p0'
      console.log(chaveCriptografica)
      const dadosCriptografados = await criptacaoDados(key, value, chaveCriptografica)
      console.log(dadosCriptografados)
      const sucessoNoArmazenamentos = await armazenamentoDados(dadosCriptografados)
